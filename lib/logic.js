...
async function createContract(contract) {
  ...
  try {
    ...
    const newContract = factory.newResource(
      NS,
      "Contract",
      contract.contractId
    );
    ...
    newContract.grower = factory.newRelationship(
      NS,
      "Grower",
      contract.grower.userId
    );
    newContract.batch = factory.newRelationship(NS, "Batch", contract.batchId);
    newBatch.contract = factory.newRelationship(
      NS,
      "Contract",
      contract.contractId
    );
  ...
  try {
    ...
    if (farmInspector) {
      ...
      await checkParticipantExists(
        farmInspectorRegistry,
        tx.farmInspector.userId
      );
      await checkParticipantRole(tx.farmInspector, "FARMINSPECTOR");
      tx.currentContract.farmInspector = factory.newRelationship(
        NS,
        "FarmInspector",
        tx.farmInspector.userId
      );
    ...
    if (shipper) {
      ...
      tx.currentContract.shipper = factory.newRelationship(
        NS,
        "Shipper",
        tx.shipper.userId
      );
    ...
    if (processor) {
      ...
      tx.currentContract.processor = factory.newRelationship(
        NS,
        "Processor",
        tx.processor.userId
      );
...
async function inspectBatch(tx) {
  ...
  try {
    ...
    await checkAssetExists(
      batchRegistry,
      tx.currentContract.batch.batchId,
      "batch"
    );
    ...
    await checkParticipantExists(
      farmInspectorRegistry,
      tx.currentContract.farmInspector.userId
    );
    await checkParticipantRole(
      tx.currentContract.farmInspector,
      "FARMINSPECTOR"
    );

    const updatedBatch = await batchRegistry.get(
      tx.currentContract.batch.batchId
    );
...
async function shipBatch(tx) {
  ...
  try {
    ...
    await checkParticipantExists(
      shipperRegistry,
      tx.currentContract.shipper.userId
    );
...
async function processBatch(tx) {
  ...
  try {
    ...
    await checkParticipantExists(
      processorRegistry,
      tx.currentContract.processor.userId
    );
